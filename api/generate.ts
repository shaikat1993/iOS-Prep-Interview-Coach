...
async function attemptGenerate(topicQuery: string, keyIndex: number, attempts: number): Promise<any> {
  ...
  try {
    ...
    return {
      ...
      diagramData: data.diagramData
        ? { nodes: data.diagramData.nodes || [], links: data.diagramData.links || [] }
        : undefined,
...
export default async function handler(req: any, res: any) {
  ...
  if (!topicQuery || typeof topicQuery !== "string") {
    return res.status(400).json({ error: "topicQuery is required" });
  }

  try {
    // Pick a random starting key so load spreads across serverless instances
    const startIndex = Math.floor(Math.random() * Math.max(API_KEYS.length, 1));
    const result = await attemptGenerate(topicQuery, startIndex, 0);
